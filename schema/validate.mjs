...
function selftest() {
  ...
  const cases = [
    ...
    // OSS-0-05 — repo_status_ref is a known, optional cross-reference to a
    // repo's x-k0nsult.status (k0nsult-eu-shield SPEC SS4.1.1); it must not
    // trip the additionalProperties parity check, and does not itself
